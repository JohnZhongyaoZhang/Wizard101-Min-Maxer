...
class WizMath:
    ...
    def critchance(self,criticalRating,blockRating,casterlevel=160):
        if criticalRating == 0 and blockRating == 0:
            return 0
        match self.crit:
            case "PvE":
    ...
    def blockchance(self,criticalRating,blockRating,casterlevel=160,receiverlevel=160):
        if criticalRating == 0 and blockRating == 0:
            return 0
        match self.crit:
            case "PvE":
                return .4 * (1-(min(casterlevel/100,1) * (3*criticalRating)/(3*criticalRating+blockRating)))
            case "PvP":
                return min(receiverlevel/185,1) * blockRating/(blockRating + 12*criticalRating)
            case "None":
                return 0
    
    def effectivecrit(self,criticalRating,blockRating,casterlevel=160,receiverlevel=160):
        if self.crit == "None":
            return 1
    ...
    def effectiveMultipllier(self,criticalRating,blockRating, damageRating, resistRating, pierceRating, casterlevel=160,receiverlevel=160):
...
def main():
    ...
    print(testing.effectiveMultipllier(criticalRating=2,blockRating=1,damageRating=0,resistRating=0,pierceRating=0,casterlevel=160,receiverlevel=160))
main()